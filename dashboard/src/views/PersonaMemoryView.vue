<template>
  <div class="grid metric-grid compact-metrics">
    ...
    <article class="panel metric">
      <strong>{{ enabledPersonas }}</strong>
      <span>Enabled personas</span>
    </article>
    <article class="panel metric">
      <strong>{{ memories.length }}</strong>
      <span>Visible memories</span>
    </article>
    <article class="panel metric">
  ...
  <div class="workbench-grid">
    <section class="panel">
      <div class="section-title">
        <div>
          ...
          <p>{{ filteredPersonas.length }} personas in {{ workspaceId }}.</p>
        </div>
        <div class="toolbar">
      ...
      <EmptyState v-if="filteredPersonas.length === 0" title="No personas" detail="Create a persona or adjust the workspace and search filters." />
    ...
    <aside class="panel detail-panel">
      <div class="section-title">
        <div>
          <h2>{{ selectedPersona?.name ?? 'Persona Detail' }}</h2>
          <p>{{ selectedPersona ? selectedPersona.id : 'Select a persona to inspect injection inputs.' }}</p>
        </div>
      </div>

      <EmptyState v-if="!selectedPersona" title="No persona selected" detail="Choose a row to inspect tone, boundaries, and scoped agents." />
  ...
  <div class="workbench-grid">
    <section class="panel">
      <div class="section-title">
        <div>
          ...
          <p>{{ filteredMemories.length }} memories visible for the current scope.</p>
        </div>
        <div class="toolbar">
      ...
      <EmptyState v-if="filteredMemories.length === 0" title="No visible memories" detail="Save a memory or adjust scope filters to list scoped records." />
      <div v-else class="table-wrap">
        <table class="table">
          <thead>
            <tr>
    ...
    <aside class="panel detail-panel">
      <div class="section-title">
        <div>
          <h2>{{ selectedMemory ? selectedMemory.type : 'Memory Detail' }}</h2>
          <p>{{ selectedMemory ? selectedMemory.id : 'Select a memory to inspect scope and retrieval signals.' }}</p>
        </div>
      </div>

      <EmptyState v-if="!selectedMemory" title="No memory selected" detail="Choose a memory row or run a search." />
      <div v-else class="detail-list">
        <div class="detail-item">
          <span>Content</span>
      ...
      <div v-if="searchResults.length" class="search-results">
        <div class="section-title compact">
          <div>
            ...
            <p>{{ searchResults.length }} matched records.</p>
          </div>
        </div>
        <button
...
import { dashboardApi, type MemoryRecord, type MemoryScope, type MemorySearchResult, type MemoryType, type Persona, type PersonaUpsertDto } from '../api/dashboard';
...
const selectedPersona = computed(() => personas.value.find((persona) => persona.id === selectedPersonaId.value) ?? filteredPersonas.value[0] ?? null);
